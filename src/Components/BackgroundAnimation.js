...
const BackgroundAnimation = () => {
  return (
    <div className="absolute inset-0 z-0 overflow-hidden bg-zinc-900"> {/* Grundfärg för bakgrunden */}
      ...
      <div
        className="absolute -top-40 -left-40 w-[1200px] h-[550px] bg-purple-500 mix-blend-screen filter blur-3xl opacity-70 animate-blob-1"
        style={{ borderRadius: '40% 60% 70% 30% / 60% 40% 30% 70%' }}
      ></div>

      {/* Blob 2: Placerad i övre högra området, lite mindre */}
      <div
        className="absolute -top-20 right-1 w-[700px] h-[1000px] bg-pink-500 mix-blend-screen filter blur-3xl opacity-70 animate-blob-2"
        style={{ borderRadius: '55% 45% 35% 65% / 45% 65% 55% 35%' }}
      ></div>

      {/* Blob 3: Störst, placerad nära mitten/nedre vänstra området */}
      <div
        className="absolute top-1/3 left-1/4 w-[750px] h-[700px] bg-cyan-500 mix-blend-screen filter blur-3xl opacity-70 animate-blob-1 animation-delay-2000"
      ...
      <div
        className="absolute bottom-1 left-[10%] w-[1000px] h-[900px] bg-green-400 mix-blend-screen filter blur-3xl opacity-70 animate-blob-gentle-float animation-delay-5000"
